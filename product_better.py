...
def Better_product_rec(list_product, df):
    ...
    for product in list_product:
        replaced, ideal_product, ideal_nutriscore, ideal_code, old_score, old_gr, new_gr =\
         find_healthier_product(product, df, dic_tag)

        if replaced & (type(ideal_product[0]) == str) & (type(ideal_product[-1]) == str) :
